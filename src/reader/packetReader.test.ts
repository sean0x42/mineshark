...
describe("PacketReader", () => {
  ...
  describe("readVarInt", () => {
    it("should read variable ints", () => {
      // Given
      const buffer = createDummyBuffer([0xf4, 0x05]);
      const reader = new PacketReader(buffer);
  ...
  describe("readUnsignedShort", () => {
    it("should read unsigned shorts", () => {
      // Given
      const buffer = createDummyBuffer([0x63, 0xde]);
      const reader = new PacketReader(buffer);
  ...
  describe("readLong", () => {
    it.skip("should read longs", () => {
      ...
      const buffer = createDummyBuffer([
        0xf4, 0x05, 0x0d, 0x31, 0x39, 0x32, 0x2e, 0x31,
      ]);
      const reader = new PacketReader(buffer);
  ...
  describe("readString", () => {
    it("should read strings", () => {
      // Given
      const buffer = createDummyBuffer([
        0x0d, 0x31, 0x39, 0x32, 0x2e, 0x31, 0x36, 0x38, 0x2e, 0x32, 0x30, 0x2e,
        0x33, 0x31,
      ]);
      const reader = new PacketReader(buffer);
